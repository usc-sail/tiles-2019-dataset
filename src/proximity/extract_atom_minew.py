...
if __name__ == '__main__':
    ...
    for participant_id in participant_list:
        ...
        if 'DS' in id:
            continue
        if os.path.exists(os.path.join(delevery_root_path, id, 'AtomProximity', id + '_AtomMinew.csv')) is False:
            continue
        ...
        # start_date1
        minew_df = pd.read_csv(os.path.join(delevery_root_path, id, 'AtomProximity', id + '_AtomMinew.csv'))
        minew_df = minew_df.sort_index()
        timestamp_col = 'Timestamp' if 'Timestamp' in list(minew_df.columns) else 'timestamp'
        
        make_dir(os.path.join(output_dir, 'atomproximity'))
        make_dir(os.path.join(output_dir, 'atomproximity', 'interaction'))
        ...
        with open(os.path.join(output_dir, 'atomproximity', 'interaction', participant_id + '.csv'), 'w') as csv_output:
        ...
        save_df = pd.read_csv(os.path.join(output_dir, 'atomproximity', 'interaction', participant_id + '.csv'))
        save_df.to_csv(os.path.join(output_dir, 'atomproximity', 'interaction', participant_id + '.csv.gz'), index=False, compression='gzip')
        os.remove(os.path.join(output_dir, 'atomproximity', 'interaction', participant_id + '.csv'))
